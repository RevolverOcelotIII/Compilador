...
char *print_categoria[] = {"N/A", "procedimento", "parametro", "vGlobal", "vLocal", "prototipo"};
...
void ImprimirTabela() {
    printf("\nTabela de Símbolos:\n");
    printf("------------------------------------------------------------------------------------------------------------------------------------\n");
    printf("| %-2s | %-10s | %-6s | %-8s | %-6s | %-8s | %-8s | %-10s | %-5s | %-5s | %-8s | %-8s | %-6s | %-8s |\n",
           "Índice", "Lexema", "Tipo", "Escopo", "Categoria", "Passagem", "Array", "Dim1", "Dim2", "Const", "Val_Const", "Zombie", "Endereço", "Rótulo");
    printf("------------------------------------------------------------------------------------------------------------------------------------\n");
    for (int i = 0; i <= tabela_idef.tamTab; i++) {
        printf("| %-2d | %-10s | %-6s | %-8s | %-6s | %-8s | %-8s | %-10d | %-5d | %-5s | %-8s | %-8s | %-6d | %-8d |\n",
               i,
               tabela_idef.tabela_simb[i].lexema,
               print_tipo[tabela_idef.tabela_simb[i].tipo],                  // Tipo (int, real, char, bool)
               print_escopo[tabela_idef.tabela_simb[i].escopo],              // Escopo (local, global)
               print_categoria[tabela_idef.tabela_simb[i].categoria],        // Categoria (procedimento, parametro, etc.)
               print_passagem[tabela_idef.tabela_simb[i].passagem],          // Passagem (referência, valor)
               ...
               tabela_idef.tabela_simb[i].is_const ? "Sim" : "Não",             // Constante (Sim/Não)
...
int VerificarSimboloRepetido(TabSimb ultimoSimbolo) {
    if (ultimoSimbolo.lexema[0] == '\0') return 0;
    for (int i = 0; i < tabela_idef.tamTab; i++) {
        if (strcmp(tabela_idef.tabela_simb[i].lexema, ultimoSimbolo.lexema) == 0) {
            if (tabela_idef.tabela_simb[i].escopo == ultimoSimbolo.escopo && !tabela_idef.tabela_simb[i].zombie) return 1;
        }
    }
    return 0;
}
...
int InserirTabela (TabSimb ultimoSimbolo) {
    ...
    if (VerificarSimboloRepetido(ultimoSimbolo)) {
        ...
        snprintf(mensagem_erro, sizeof(mensagem_erro), "Erro: Identificador '%s' já declarado no escopo atual.", ultimoSimbolo.lexema);
        erro(mensagem_erro);
    }
...
void EditarLinhaTabela(int i, TabSimb simbolo) {
    if(sizeof(simbolo.lexema) > 0) strcpy(tabela_idef.tabela_simb[i].lexema, simbolo.lexema);
    tabela_idef.tabela_simb[i].zombie = simbolo.zombie;
    if(simbolo.categoria) tabela_idef.tabela_simb[i].categoria = simbolo.categoria;
